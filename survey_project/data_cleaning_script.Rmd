...
```{r}
#survey_data$local_authority <- local_authority_data
```
...
```
```{r}
# list of transport type categories 
...
$", "other") %>% 
  ...
  replacement_function("^city of edinburgh$", "edinburgh")
```
...
```{r}
# reading in a file with information about population of Scottish places greater than 500
...
location_and_population_data <- right_join(locality_data, survey_data, by = c("location"= "overnight_stay_location"))  %>% 
  select(location, population)
location_and_population_data
```
```{r}
# adding the population variable to survey data 
survey_data$population <- location_and_population_data$population
```
...
```{r}
unique_location_and_popualtion_data <- unique(location_and_population_data) %>% 
  filter(location != "england", location!= "other", location != "multiple", location != "strathearn", location != "local")
```

```{r}
map_data <- unique_location_and_popualtion_data %>% 
  select(location)
```
...
```
```{r}
map_data <- paste0(map_data$location, "+scotland") 
map_data <- str_replace_all(map_data, " ", "+")
```

```{r}
locality_data <- as_tibble(locality_data)
```
```{r}
...
location_distance_count_data <- left_join(distances_data_frame, survey_data, by = c("location" = "overnight_stay_location")) %>% 
  group_by(location) %>% 
  mutate(count = n()) %>% 
  unique() %>% 
  mutate( 
    kilometres = round(as.numeric(distance)/1000, 1), 
    miles = round(kilometres * 0.621371, 1)) %>% 
  select(-distance)
